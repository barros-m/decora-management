  ...
  return (
    <section className="rounded-[2rem] border border-primary/15 bg-white/92 p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">Coming soon</span>
      </div>
      {children}
    </section>
  );
...
function ComingSoonTab({ tabName }: { tabName: string }) {
  return (
    <div className="rounded-[2rem] border border-primary/15 bg-white/92 p-8">
      <h2 className="text-3xl font-semibold tracking-tight text-stone-900">
        {tabName} is coming soon
      </h2>
      <p className="mt-2 max-w-2xl text-stone-600">
        This section is scaffolded and ready for implementation. Navigation is wired, and content placeholders are in place.
      </p>
    </div>
  );
}
  ...
  return (
    <>
      <header>
        ...
        <h1 className="text-4xl font-semibold tracking-tight text-stone-900">Welcome back, {userName}</h1>
        <p className="mt-1 text-stone-600">Here&apos;s what&apos;s blooming today in Florida.</p>
      </header>

      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard title="Total Inquiries" value="124" note="+12% from last month" tone="primary" />
        <StatCard title="Pending Follow-ups" value="18" note="Action required" tone="warn" />
        <StatCard title="Upcoming Events" value="12" note="Next: Palm Beach Gala" tone="success" />
        <StatCard title="This Month Booked" value="8" note="On track for target" tone="muted" />
      </div>
...
export function DashboardContent({ activeTab, userName }: DashboardContentProps) {
  if (activeTab === "dashboard") return <DashboardHome userName={userName} />;
  if (activeTab === "inquiries") return <ComingSoonTab tabName="Inquiries" />;
  if (activeTab === "events") return <ComingSoonTab tabName="Events" />;
  return <ComingSoonTab tabName="Vendors" />;
}
